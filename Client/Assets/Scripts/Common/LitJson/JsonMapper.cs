    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata (Type type)
        {
            ...
            foreach (var p_info in type.GetProperties ()) {
                if (p_info.Name == "Item") {
                    ...
                    data.DictionaryKeyType = parameters[0].ParameterType;
                    data.ElementType = p_info.PropertyType;
            ...
            foreach (var f_info in type.GetFields(fieldFlags))
            {
                var p_data = new PropertyMetadata ();
                p_data.Info = f_info;
                p_data.IsField = true;
        ...
        private static object ReadValue (Type inst_type, JsonReader reader)
        {
            ...
            if (reader.Token == JsonToken.Double ||
                ...
                if (inst_type.IsEnum)
                {
                    if (reader.Value.GetType() == typeof(string))
                        return Enum.Parse(inst_type, (string)reader.Value, true);
                    else
                ...
                if (inst_type == typeof(System.UInt64))
                {
                ...
                else if (inst_type == typeof(System.UInt32))
                {
                    return Convert.ToUInt32(reader.Value);
                }
                else if (inst_type == typeof(System.Int32))
                {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                {
                    //try read array as object
                    AddObjectMetadata (inst_type);
                    var t_data1 = object_metadata[inst_type];
                    ...
                    while (true) {
                        ...
                        if (t_data1.Properties.ContainsKey (property)) {
                            var prop_data =
                                t_data1.Properties[property];
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (t_data.DictionaryKeyType == typeof(int))
                        {
                            var k = int.Parse(property);
                            var v = ReadValue(t_data.ElementType, reader);
                            ((IDictionary) instance).Add(k, v);
                        }
                        else if (t_data.DictionaryKeyType.IsEnum)
                        {
                            ((IDictionary) instance).Add (Enum.Parse(t_data.DictionaryKeyType, property), ReadValue (t_data.ElementType, reader));
                        }
                        else
                        {
                                        ...
                                        int depth)
        {
            ...
            foreach (var p_data in props) {
                if (p_data.IsField) {
                    ...
                    var writeObj = ((FieldInfo) p_data.Info).GetValue(obj);
                    WriteValue (writeObj, writer, writer_is_private, depth + 1);
                }
                else {
